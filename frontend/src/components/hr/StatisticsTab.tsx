...
function buildPrintableReportHtml(args: {
  ...
  const filenameBase = `bao-cao-thuc-tap-${format(new Date(), 'yyyy-MM-dd')}`
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .toolbar {
      ...
      padding: 12px 16px; background: #ffffff; border-bottom: 1px solid #d1d5db;
    ...
    .page { width: 210mm; min-height: 297mm; padding: 16mm; margin: 18px auto; background: #fff; }
    ...
    @media print {
      body { background: #fff; }
      .page { width: auto; min-height: auto; padding: 12mm; margin: 0; }
  ...
  const handleExport = async (type: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'pdf' && pdfWindow) {
        ...
        toast({
          title: 'Đã mở báo cáo trên trình duyệt',
          description: 'Bấm nút “Tải xuống PDF” ở đầu trang báo cáo để lưu file.',
        })
      } else {
